 ...
 */
 class Payment extends FRONTEND_Controller{
    ...
    {		
		...
		$fees = $this->tfp->get_student_fee($row['id']);
		$fee = array();
		foreach($fees as $val){
			$data['id'] = $val->id;
	...
	public function download($pid)
	{
		...
		$html .='<table width="80%" align="center" class="receipt_head">		
					<tr>
						...
						<td>'.$this->current_date().'</td>	
					</tr>	
					<tr>
						<td>Student Name</td>
						<td colspan="4">: '.$row["first_name"].' '.$row["last_name"].'</td>					
					</tr>
					<tr>
					...
					<tr>
						<td>Month</td>
						<td colspan="4">: '.date("F", mktime(0, 0, 0, $payment["month"], 10)).'</td>					
					</tr>
					<tr>
