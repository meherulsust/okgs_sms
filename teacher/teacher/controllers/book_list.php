 ...
 */
 class Book_list extends FRONTEND_Controller{
	...
	{		
		...
		$row = $this->teachermodel->find_row(array('id'=>$user_id));
		$book_list = $this->teachermodel->get_book_list($row['class_id']);
	...
	{		
		...
		$book_list = $this->teachermodel->get_book_list($class_id);
		$get_class = $this->teachermodel->get_class();
		$this->tpl->assign('get_class',$get_class);
		$this->tpl->assign('book_list',$book_list);		
	...
	public function download($class_id)
	{
		...
		$book_list = $this->teachermodel->get_book_list($class_id);
		$class_title = $this->teachermodel->get_class_title($class_id);
	...
	function get_notebook()
	{
		...
		$notebook_list = $this->teachermodel->get_notebook($data);
	...
	public function note_download($file_name)
	{
		...
		$data = file_get_contents($final_url."/smsadmin/uploads/note_book/".$file_name); 
		$name = $file_name;
