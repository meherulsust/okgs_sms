...
//        $students = $this->get_student_info($class);
   ...
   public function get_student_min_info($id){
    ...
    public function get_student_payment($pid){
        $this->db->select('student_tuition_fee_payment_id,bank_transection_id');
        $this->db->from('student_payment_transection');
        $this->db->where('student_tuition_fee_payment_id', $pid);
        $query = $this->db->get();
        $rs = $query->row_array();
        return $rs;
    }
