...
//        $this->tpl->assign('student_details', $students_details);
    ...
    function popup_2017_result($class_id, $student_id, $section_id, $exam_id){
        $this->load->model('schoolmodel');
        $school_info = $this->schoolmodel->find(1);
        $classes = $this->publish_resultmodel->get_classes();
        $this->tpl->assign('classes', $classes);
...
//        $this->tpl->assign('exams', $exams);
        ...
        $student_name = $this->publish_resultmodel->get_student_name($student_id);
        $section = $this->publish_resultmodel->get_section($student_id);
        $class_title = $this->publish_resultmodel->get_class_title($class_id);
        $student_house = $this->publish_resultmodel->get_house($student_id);
        $student_number = $this->publish_resultmodel->get_student_number($student_id);
        ...
        $data['activities'] = $this->publish_resultmodel->get_student_pen_pic($student_id, $exam_id);

        $rs = $this->publish_resultmodel->get_entire_class_2017_result($class_id, $exam_id, $student_id, $result_scale['result_scale_id']);

        $optional_subject = $this->publish_resultmodel->get_optional_subject($student_id);
