...
class Sylabus extends BACKEND_Controller{
	...
	protected function init_grid()
	{
	   ...
	   $this->grid_board->set_title('Syllabus list');
           $this->grid_board->add_link('New Sylabus',site_url('sylabus/create'),array('class'=>'add','id'=>'new_sylabus'));
	...
	function status($id,$stat) {
        $this->load->model('sylabusmodel');
        $row = $this->sylabusmodel->find($id);
        if (empty($row)) {
            show_404();
        }
        $this->sylabusmodel->update_status($stat,$id);
        $this->session->set_flashdata('success', "sylabus status has been changed successfully.");
        redirect('sylabus/index');
    }
	...
	protected function init_matrix_grid($sid)
	{
	   ...
	   $this->grid_board->set_action($actions);
	   $grid_columns = array('id'=>array('visible'=>false),'title'=>'Title','max_range'=>'Max Range','min_range'=>'Min Range','weight'=>'Weight','grade_title'=>'Grade Title','created_at'=>array('title'=>'Create Date','datetime'=>true));
	   $this->grid_board->set_column($grid_columns);
	...
	public function smdel($id)
	{
		$this->load->model('scalematrixmodel','smmodel');
		$this->smmodel->delete($id);
		$this->session->set_flashdata('success',"scale Range has been deleted successfully");
		redirect('sylabus/scale');
	}
