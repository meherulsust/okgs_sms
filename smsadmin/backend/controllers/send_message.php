...
class Send_message extends BACKEND_Controller {
    ...
    function index() {
        ...
        $username='imsnadmin';
        $password='imsn321';
        $sms_quantity = json_decode(file_get_contents('http://202.126.120.118/smsenquiry/index.php/sms_api/get_user_account_info?username=imsnadmin'));
        $data['total_purchage_sms'] = $sms_quantity->total_purchage_sms;
        $data['total_available_sms'] = $sms_quantity->total_available_sms;
        $data['total_sent_sms'] = $sms_quantity->total_purchage_sms - $sms_quantity->total_available_sms;
    ...
    public function save_send_message() {
        ...
        foreach ($this->input->post('student_number') as $lm => $val) {
            ...
            $result = $this->send_sms_imsn($gsm, $full_message);
            if ($result == 1) {
                $data['status'] = 1;
            } else {
                $data['status'] = 0;
