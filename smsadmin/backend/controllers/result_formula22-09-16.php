...
//    }
    ...
    public function popup_result($class_id, $student_id, $section_id, $exam_id) {
        ...
        if(!empty($configures)){
            //$fields = explode(',', $configures['field_name']);
            $subjects = $exam_title;
        ...
        if(!empty($exam_id) && $exam_id == '3'){
            $half_yearly_results = $this->publish_resultmodel->get_half_yearly_results(2, $class_id);            
        }
            ...
            if(!empty($students_detail)){
                ...
                $data['class_id'] = $students_detail['class_id'];
                $data['exam_id'] = $students_detail['exam_id'];
...
//                print_r($rs);
            ...
            if(!empty($results)){                
                //Calculate Half yearly result in Final Examination
                if(!empty($half_yearly_results)){
                    for($i=0;$i<count($results);$i++) {
                        $cal_avg = ($half_yearly_results[$i]['mid_term'] + $results[$i]['yearly_grand_total'])/2;
                        $scale_matrix = $this->publish_resultmodel->get_scale_matrices($result_scale['result_scale_id'], $cal_avg);
                        $results[$i]['final_lg'] = $scale_matrix['title'];
                        $results[$i]['final_gp'] = $scale_matrix['weight'];
                        $results[$i]['mid_term_mks'] = $half_yearly_results[$i]['mid_term'];                        
                    }
                }               
            }
//            print_r($asd);
