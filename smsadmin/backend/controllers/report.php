...
class Report extends BACKEND_Controller {
    ...
    public function edit_payment_status($id)
    {
        ...
        if ($info['pay_status'] == 'UNPAID') {
            ...
            if ($this->editpaymentstatusform->validate()) {
                $data1['student_tuition_fee_payment_id']= $id;
                $data1['bank_transection_id']= $this->editpaymentstatusform->get_value('bank_transection_id');	
                $data1['created_at']= date("Y-m-d h:i:s");	
                $data1['created_by'] = $this->session->userdata('user_id');
                $transection = $this->tfpm->transection_data($data1);
	...
	/*------ student information download by quader -------*/
	function download_student_info(){
        ...
        $this->tpl->assign('report_list', $std_info); 

		$report_title = 'Report for Student of : ' . 'imsn';
        $this->tpl->assign('report_title', $report_title);
        $report_file_name = 'Report_for_student_info_of_' . 'imsn' . '_' . date('y-m-d') . '.xls';
        $this->tpl->assign('report_file_name', $report_file_name);
	}
