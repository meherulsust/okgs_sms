...
//            'del' => array('title' => 'Delete', 'action' => 'del', 'controller' => '', 'tips' => 'Delete this exam'),
    ...
    function edit($id = '') {
        ...
        $this->tpl->assign('exam_info', $info);   
        $this->load->form('examform', null, $info);
        
        $this->process_form($this->examform);
    }
    public function amend_result() {
        $this->load->form('examform');
        $this->tpl->set_view('edit_exam');
        $this->edit_exam($this->examform);
    }
    public function edit_exam($form) {
        if ($form->validate()) {
        $this->load->model('exammodel');
        
        $data['title'] = $this->input->post('exam_title');
        $data['class_id'] = $this->input->post('exam_class_id');
        $data['section_id'] = $this->input->post('exam_section_id');
        $data['fee'] = $this->input->post('exam_fee');
        $data['exam_session'] = $this->input->post('exam_exam_session');
        $data['exam_type_lookup_id'] = $this->input->post('exam_exam_type_lookup_id');
        $data['is_final'] = $this->input->post('exam_is_final');
        $data['start_date'] = date('Y-m-d', strtotime($this->input->post('exam_sdatepicker')));
        $data['end_date'] = date('Y-m-d', strtotime($this->input->post('exam_edatepicker')));
        $data['is_result_publish'] = $this->input->post('exam_is_result_publish');
        $exam_id = $this->input->post('exam_id');
        
        $edit_exam = $this->exammodel->edit_exam($exam_id, $data);
        
        $this->session->set_flashdata('success', "Exam has been edited successfully");
        redirect('exam/index');
        }
    }
    ...
    protected function init_registration_grid($eid) {
        ...
        $this->grid_board->set_action($actions);
        $grid_columns = array('id' => array('visible' => false), 'student' => 'Student', /*'fee_received' => 'Fee Received'*/'class_roll'=>'Class Roll', 'status' => array('title' => 'Status', 'status' => 'exam/registat'), 'created_at' => array('title' => 'Create Date', 'datetime' => true));
        $this->grid_board->set_column($grid_columns);
