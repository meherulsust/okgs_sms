...
<form name='frm-generate-grid-board' id='frm-generate-grid-board' method='post' action='<?php echo site_url($active_module.'/generate_grid_board'); ?>' >
...
<script>
$(document).ready(function(){
    $('#generate_grid_boardform_class_id').change(function(e){
        var class_id = $("#generate_grid_boardform_class_id").val();
        $("#generate_grid_boardform_subject_id").find("option:gt(0)").remove();
        $.ajax({
            ...
            success: function(data){
                $.each(data,function(i,v){
                    $("#generate_grid_boardform_subject_id").append('<option value="'+ v.id +'">'+ v.title +'</option>');
                });
            }
        });  
    });
