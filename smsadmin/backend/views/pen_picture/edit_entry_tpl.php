...
<div id="box">
    <h3 class='grid_title_bar'>Update Template</h3>
<form name='frm-update-grid-board' id='frm-update-grid-board' method='post' action='<?php echo site_url($active_module.'/update_pen_pictures')?>' >
...
//            print_r($form_elements);
    ...
    <table width='100%' class="result_entry_table">
        ...
        <tr>
            ...
            <td>
                <textarea name="pen_picture_temaplte_id<?php echo $x; ?>" id="pen_picture_temaplte_id<?php echo $x; ?>" style="background: none; height: 40px; width: 200px; margin-bottom: 2px;" placeholder="Your comment here"><?php if(!empty($form_element['pen_picture_temaplte_id'])){echo $form_element['pen_picture_temaplte_id'];}else{echo '';}?></textarea>
            </td>
            <td>
                <select class="activity_select_input" name="discipline<?php echo $x; ?>" id="discipline<?php echo $x; ?>">
                    ...
                    ?>
                    <option <?php if($key == $form_element['discipline']){echo 'selected="selected"';}else{echo '';} ?>  value="<?php echo $key; ?>"><?php echo $activity; ?></option>
                    <?php } ?>
                </select>
            </td>
            <td>
                <select class="activity_select_input" name="cleanliness<?php echo $x; ?>" id="cleanliness<?php echo $x; ?>">
                    ...
                    <option <?php if($key == $form_element['cleanliness']){echo 'selected="selected"';}else{echo '';} ?> value="<?php echo $key; ?>"><?php echo $activity; ?></option>
                    <?php } ?>
                </select>
            </td>
            <td>
                <select class="activity_select_input" name="co_curricular_activities<?php echo $x; ?>" id="co_curricular_activities<?php echo $x; ?>">
                    <option value="0">--Select--</option>
                    <?php foreach($activities as $key=>$activity){  ?>
                    <option <?php if($key == $form_element['co_curricular_activities']){echo 'selected="selected"';}else{echo '';} ?> value="<?php echo $key; ?>"><?php echo $activity; ?></option>
                    <?php } ?>
                </select>
            </td>
        </tr>
        <input type="hidden" name="student_id<?php echo $x; ?>" id="student_id<?php echo $x; ?>" value="<?php echo $form_element['student_id']; ?>" />
        <input type="hidden" name="pen_details_id<?php echo $x; ?>" id="pen_details_id<?php echo $x; ?>" value="<?php echo $form_element['pen_detail_id']; ?>" />
