...
class Pen_picturemodel extends BACKEND_Model{
    ...
    public function get_students_ids($class_id, $section_id, $exam_id){
         $this->db->select('student_id');
        $this->db->where('class_id', $class_id);
        $this->db->where('section_id', $section_id);
        $this->db->where('exam_id', $exam_id);
        $rs = $this->db->get('student_pen_picture_details');
        return $rs->result_array();
    }
    public function get_template_info($student_id, $class_id, $section_id, $exam_id) {
        $this->db->select('*');
        $this->db->where('student_id', $student_id);
        $this->db->where('class_id', $class_id);
        $this->db->where('section_id', $section_id);
    ...
    public function check_penpic_entry($class_id, $section_id, $exam_id) {
        $this->db->select('id');
        $this->db->where('class_id', $class_id);
        $this->db->where('section_id', $section_id);
        $this->db->where('exam_id', $exam_id);
